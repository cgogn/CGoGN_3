...
#ifndef ACC_BVHTREE_SPHERES_HEADER
#define ACC_BVHTREE_SPHERES_HEADER
...
class BVHTreeSpheres
{
public:
	typedef std::shared_ptr<BVHTreeSpheres> Ptr;
	typedef std::shared_ptr<const BVHTreeSpheres> ConstPtr;
...
private:
	...
	typedef acc::Sphere<Vec3fType> Sphere;
...
public:
	static Ptr create(std::vector<Vec3fType> const& centers, std::vector<double> const& radii,
					  int max_threads = std::thread::hardware_concurrency())
	{
		return std::make_shared<BVHTreeSpheres>(centers, radii, max_threads);
	}

	template <class C>
	static C convert(BVHTreeSpheres const& bvh_tree);
	 ...
	 *
	 * The spheres should be given as two vectors
	 * containing the centers and the radii. */
	BVHTreeSpheres(std::vector<Vec3fType> const& centers, std::vector<double> const& radii,
				   int max_threads = std::thread::hardware_concurrency());

	Sphere const& get_sphere(IdxType idx) const
	{
		return spheres[idx];
	}
...
template <typename IdxType, typename Vec3fType>
constexpr IdxType BVHTreeSpheres<IdxType, Vec3fType>::NAI;

template <typename IdxType, typename Vec3fType>
void BVHTreeSpheres<IdxType, Vec3fType>::split(typename Node::ID node, std::vector<AABB> const& aabbs,
											   std::atomic<int>* num_threads)
{
	...
	if ((*num_threads -= 1) >= 1)
	{
		...
		{
			std::thread other(&BVHTreeSpheres::split, this, left, std::cref(aabbs), num_threads);
...
template <typename IdxType, typename Vec3fType>
std::pair<typename BVHTreeSpheres<IdxType, Vec3fType>::Node::ID, typename BVHTreeSpheres<IdxType, Vec3fType>::Node::ID>
BVHTreeSpheres<IdxType, Vec3fType>::sbsplit(typename Node::ID node_id, std::vector<AABB> const& aabbs)
{
#if BVHTREE_NUM_BINS
...
template <typename IdxType, typename Vec3fType>
std::pair<typename BVHTreeSpheres<IdxType, Vec3fType>::Node::ID, typename BVHTreeSpheres<IdxType, Vec3fType>::Node::ID>
BVHTreeSpheres<IdxType, Vec3fType>::bsplit(typename Node::ID node_id, std::vector<AABB> const& aabbs)
{
	Node& node = nodes[node_id];
	IdxType n = node.last - node.first;
...
template <typename IdxType, typename Vec3fType>
std::pair<typename BVHTreeSpheres<IdxType, Vec3fType>::Node::ID, typename BVHTreeSpheres<IdxType, Vec3fType>::Node::ID>
BVHTreeSpheres<IdxType, Vec3fType>::ssplit(typename Node::ID node_id, std::vector<AABB> const& aabbs)
{
	Node& node = nodes[node_id];
	IdxType n = node.last - node.first;
...
template <typename IdxType, typename Vec3fType>
BVHTreeSpheres<IdxType, Vec3fType>::BVHTreeSpheres(std::vector<Vec3fType> const& centers,
												   std::vector<double> const& radii, int max_threads)
	: num_nodes(0)
{

	std::size_t num_spheres = centers.size();
	std::vector<AABB> aabbs(num_spheres);
	std::vector<Sphere> tspheres(num_spheres);

	/* Initialize vector with upper bound of nodes. */
	nodes.resize(2 * num_spheres - 1);

	/* Initialize root node. */
	Node& root = nodes[create_node(0, num_spheres)];
	for (std::size_t i = 0; i < aabbs.size(); ++i)
	{
		tspheres[i].center = centers[i];
		tspheres[i].radius = radii[i];

		calculate_aabb(tspheres[i], &aabbs[i]);
		root.aabb += aabbs[i];
	}

	indices.resize(num_spheres);
	...
	spheres.resize(tspheres.size());
	for (std::size_t i = 0; i < indices.size(); ++i)
		spheres[i] = tspheres[indices[i]];
}

// template <typename IdxType, typename Vec3fType>
// bool BVHTreeSpheres<IdxType, Vec3fType>::intersect(Ray const& ray, typename Node::ID node_id, Hit* hit) const
// {
// 	Node const& node = nodes[node_id];
// 	bool ret = false;
// 	for (std::size_t i = node.first; i < node.last; ++i)
...
// template <typename IdxType, typename Vec3fType>
// bool BVHTreeSpheres<IdxType, Vec3fType>::intersect(Ray ray, Hit* hit_ptr) const
// {
// 	Hit hit;
// 	hit.t = inf;
...
template <typename IdxType, typename Vec3fType>
std::pair<IdxType, Vec3fType> BVHTreeSpheres<IdxType, Vec3fType>::closest_point(Vec3fType vertex,
																				typename Node::ID node_id) const
{
	...
	for (std::size_t i = node.first; i < node.last; ++i)
	{
		Vec3fType cp_tri = acc::closest_point(vertex, spheres[i]);
...
template <typename IdxType, typename Vec3fType>
bool BVHTreeSpheres<IdxType, Vec3fType>::closest_point(Vec3fType vertex, std::pair<IdxType, Vec3fType>* cp_ptr,
...
template <typename IdxType, typename Vec3fType>
Vec3fType BVHTreeSpheres<IdxType, Vec3fType>::closest_point(Vec3fType vertex)
{
...
#endif /* ACC_BVHTREE_SPHERES_HEADER */
