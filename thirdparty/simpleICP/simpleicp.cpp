...
Eigen::Matrix<double, 4, 4> SimpleICP(const Eigen::MatrixXd& X_fix, const Eigen::MatrixXd& X_mov,
									  const int& correspondences, const int& neighbors, const double& min_planarity,
									  const double& min_change, const int& max_iterations)
{
	...
	for (int i = 0; i < max_iterations; i++)
	{
		...
		// if (i == 0)
		// {
		// 	printf("[%s] %9s | %15s | %15s | %15s\n", Timestamp(), "Iteration", "correspondences", "mean(residuals)",
		// 		   "std(residuals)");
		// 	printf("[%s] %9d | %15d | %15.4f | %15.4f\n", Timestamp(), i, int(initial_dists.size()),
		// 		   initial_dists.mean(), Std(initial_dists));
		// }
		// printf("[%s] %9d | %15d | %15.4f | %15.4f\n", Timestamp(), i + 1, int(residual_dists.size()),
		// 	   residual_dists_mean.back(), residual_dists_std.back());
	}

	// printf("[%s] Estimated transformation matrix H:\n", Timestamp());
	// printf("[%s] [%12.6f %12.6f %12.6f %12.6f]\n", Timestamp(), H_new(0, 0), H_new(0, 1), H_new(0, 2), H_new(0, 3));
	// printf("[%s] [%12.6f %12.6f %12.6f %12.6f]\n", Timestamp(), H_new(1, 0), H_new(1, 1), H_new(1, 2), H_new(1, 3));
	// printf("[%s] [%12.6f %12.6f %12.6f %12.6f]\n", Timestamp(), H_new(2, 0), H_new(2, 1), H_new(2, 2), H_new(2, 3));
	// printf("[%s] [%12.6f %12.6f %12.6f %12.6f]\n", Timestamp(), H_new(3, 0), H_new(3, 1), H_new(3, 2), H_new(3, 3));
...
bool CheckConvergenceCriteria(const std::vector<double>& residual_dists_mean,
							  const std::vector<double>& residual_dists_std, const double& min_change)
{
