...
void update_time_step(MESH& m, Attributes<MESH>& swa, Context& swc)
{
	...
	parallel_foreach_cell(m, [&] (Face f) -> bool
	{
		...
		foreach_incident_edge(m, f, [&] (Edge ie) -> bool
		{
			...
			if (h > swc.hmin_)
				lambda = fabs((*swa.face_q_)[fidx]*(*swa.edge_normX_)[ieidx] + (*swa.face_r_)[fidx]*(*swa.edge_normY_)[ieidx]) / std::max(h, swc.hmin_) + sqrt(9.81*h);
	...
	parallel_foreach_cell(m, [&] (Face f) -> bool
	{
		...
		if ((*swa.face_area_)[fidx]*(*swa.face_phi_)[fidx]*((*swa.face_h_)[fidx]+(*swa.face_zb_)[fidx]) < (-(*swa.face_discharge_)[fidx]*min_dt_per_thread[threadidx]))
			min_dt_per_thread[threadidx] = - (*swa.face_area_)[fidx]*(*swa.face_phi_)[fidx]*((*swa.face_h_)[fidx]+(*swa.face_zb_)[fidx]) / (*swa.face_discharge_)[fidx];
		return true;
	});
...
void execute_time_step(MESH& m, Attributes<MESH>& swa, Context& swc)
{
	...
	parallel_foreach_cell(m, [&] (Edge e) -> bool
	{
		...
		if (is_incident_to_boundary(m, e)) // border conditions
		{
			...
			if ((*swa.face_phi_)[fidx] > swc.small_)
				riemann_flux = border_condition(
					(*swa.edge_bc_type_)[eidx], (*swa.edge_bc_value_)[eidx],
					(*swa.edge_normX_)[eidx], (*swa.edge_normY_)[eidx],
					(*swa.face_q_)[fidx], (*swa.face_r_)[fidx], (*swa.face_h_)[fidx]+(*swa.face_zb_)[fidx],
					(*swa.face_zb_)[fidx],
					9.81, swc.hmin_, swc.small_
				);
		}
		else // Inner cell: use the lateralised Riemann solver
		{
			...
			if ((*swa.face_h_)[f1idx] > swc.hmin_ || (*swa.face_h_)[f2idx] > swc.hmin_)
			{
				...
				Scalar qL = (*swa.face_q_)[f1idx]*(*swa.edge_normX_)[eidx] + (*swa.face_r_)[f1idx]*(*swa.edge_normY_)[eidx];
				Scalar qR = (*swa.face_q_)[f2idx]*(*swa.edge_normX_)[eidx] + (*swa.face_r_)[f2idx]*(*swa.edge_normY_)[eidx];
				Scalar rL = -(*swa.face_q_)[f1idx]*(*swa.edge_normY_)[eidx] + (*swa.face_r_)[f1idx]*(*swa.edge_normX_)[eidx];
				Scalar rR = -(*swa.face_q_)[f2idx]*(*swa.edge_normY_)[eidx] + (*swa.face_r_)[f2idx]*(*swa.edge_normX_)[eidx];
	...
	parallel_foreach_cell(m, [&] (Face f) -> bool
	{
		...
		foreach_incident_edge(m, f, [&] (Edge ie) -> bool
		{
			...
			if (fidx == (*swa.edge_left_face_index_)[ieidx])
			{
				...
				(*swa.face_q_)[fidx] -= factF * (((*swa.edge_f2_)[ieidx] + (*swa.edge_s2L_)[ieidx])*(*swa.edge_normX_)[ieidx] - (*swa.edge_f3_)[ieidx]*(*swa.edge_normY_)[ieidx]);
				(*swa.face_r_)[fidx] -= factF * ((*swa.edge_f3_)[ieidx]*(*swa.edge_normX_)[ieidx] + ((*swa.edge_f2_)[ieidx] + (*swa.edge_s2L_)[ieidx])*(*swa.edge_normY_)[ieidx]);
			}
			else
			{
				(*swa.face_h_)[fidx] += factF * (*swa.edge_f1_)[ieidx];
				(*swa.face_q_)[fidx] += factF * (( (*swa.edge_f2_)[ieidx] + (*swa.edge_s2R_)[ieidx])*(*swa.edge_normX_)[ieidx] - (*swa.edge_f3_)[ieidx]*(*swa.edge_normY_)[ieidx]);
				(*swa.face_r_)[fidx] += factF * ( (*swa.edge_f3_)[ieidx]*(*swa.edge_normX_)[ieidx] + ( (*swa.edge_f2_)[ieidx]+(*swa.edge_s2R_)[ieidx])*(*swa.edge_normY_)[ieidx]);
			}
			return true;
		});
		return true;
	...
	parallel_foreach_cell(m, [&] (Face f) -> bool
	{
		...
		if (swc.friction_ != 0)
		{
			...
			{
				qx = qx * exp(-(9.81 * sqrt(qx*qx+qy*qy) / (std::max(swc.kx_*swc.kx_, swc.small_*swc.small_) * pow((*swa.face_h_)[fidx], 7./3.))) * swc.dt_);
				qy = qy * exp(-(9.81 * sqrt(qx*qx+qy*qy) / (std::max(swc.ky_*swc.ky_, swc.small_*swc.small_) * pow((*swa.face_h_)[fidx], 7./3.))) * swc.dt_);
			}
			else
			{
				qx = 0.;
