...
public:
	...
	void start(std::string const& command)
	{
		...
		if (!CreateProcessW(nullptr, (LPWSTR)wcommand.c_str(), nullptr, nullptr, FALSE, CREATE_NEW_CONSOLE, nullptr,
							nullptr, &si, &m_pi))
...
protected:
	explicit dialog() : m_async(std::make_shared<executor>())
	{
	...
	std::string desktop_helper() const
	{
		...
		return flags(flag::has_zenity)
				   ...
				   : flags(flag::has_matedialog)
						 ? "matedialog"
						 : flags(flag::has_qarma) ? "qarma" : flags(flag::has_kdialog) ? "kdialog" : "echo";
#endif
	}
	...
	{
		return "\"" +
			   std::regex_replace(std::regex_replace(str, std::regex("[\\\\\"]"), "\\$&"), std::regex("'"), "'\\''") +
			   "\"";
	}
...
protected:
	...
	file_dialog(type in_type, std::string const& title, std::string const& default_path = "",
				std::vector<std::string> filters = {}, bool allow_multiselect = false, bool confirm_overwrite = true)
	{
#if _WIN32
		...
		m_async->start([this, in_type, title, default_path, filter_list, allow_multiselect,
						confirm_overwrite](int* exit_code) -> std::string {
			(void)exit_code;
			...
			if (in_type == type::open)
			{
				...
				for (; iter != end; ++iter)
				{
					...
					if (pat == "*" || pat == "*.*")
						...
						filter_list +=
							(filter_list.size() == 0 ? "" : ",") + osascript_quote(pat.substr(2, pat.size() - 2));
				}
		...
		{
			command += " --file-selection --filename=" + shell_quote(default_path) + " --title " + shell_quote(title) +
					   " --separator='\n'";
...
public:
	notify(std::string const& title, std::string const& message, icon icon = icon::info)
	{
		...
		auto script = "Add-Type -AssemblyName System.Windows.Forms;"
					  ...
					  std::to_string(delay) + ", " + powershell_quote(title) + ", " + powershell_quote(message) +
					  ", "
					  "'" +
					  get_icon_name(icon) +
					  "');"
		...
		if (is_osascript())
		{
			command += " -e 'display notification " + osascript_quote(message) + "     with title " +
		...
		else if (is_zenity())
		{
			command += " --notification"
					   " --window-icon " +
					   get_icon_name(icon) + " --text " + shell_quote(title + "\n" + message);
		}
		else if (is_kdialog())
		{
			command += " --icon " + get_icon_name(icon) + " --title " + shell_quote(title) + " --passivepopup " +
					   shell_quote(message) + " 5";
		}
#endif
...
{
public:
	message(std::string const& title, std::string const& text, choice choice = choice::ok_cancel,
			icon icon = icon::info)
	{
#if _WIN32
		...
		if (is_osascript())
		{
			command += " -e 'display dialog " + osascript_quote(text) + "     with title " + osascript_quote(title);
			switch (choice)
			{
			case choice::ok_cancel:
...
#define PFD_OSX_ICON(n)                                                                                                \
	...
	"& \"CoreServices:CoreTypes.bundle:Contents:Resources:" n ".icns\")"
			case icon::info:
			default:
				command += PFD_OSX_ICON("ToolBarInfo");
				break;
			case icon::warning:
				command += "caution";
				break;
			case icon::error:
				...
				break;
			case icon::question:
				command += PFD_OSX_ICON("GenericQuestionMarkIcon");
				break;
#undef PFD_OSX_ICON
			}
			command += "'";
		...
		else if (is_zenity())
		{
			...
			{
			case choice::ok_cancel:
				command += " --question --ok-label=OK --cancel-label=Cancel";
				break;
			case choice::yes_no:
				...
				break;
			case choice::yes_no_cancel:
				command += " --question --switch --extra-button No --extra-button Yes --extra-button Cancel";
				break;
			case choice::retry_cancel:
				command += " --question --switch --extra-button Retry --extra-button Cancel";
				break;
			case choice::abort_retry_ignore:
				command += " --question --switch --extra-button Abort --extra-button Retry --extra-button Ignore";
				break;
			default:
				switch (icon)
				{
			...
			command += " --title " + shell_quote(title) + " --width 300 --height 0" // sensible defaults
					   + " --text " + shell_quote(text) + " --icon-name=dialog-" + get_icon_name(icon);
		}
		else if (is_kdialog())
		{
			if (choice == choice::ok)
			...
			command += " " + shell_quote(text) + " --title " + shell_quote(title);
...
public:
	open_file(std::string const& title, std::string const& default_path = "",
			  std::vector<std::string> filters = {"All Files", "*"}, bool allow_multiselect = false)
		: file_dialog(type::open, title, default_path, filters, allow_multiselect, false)
	{
	}
...
public:
	save_file(std::string const& title, std::string const& default_path = "",
			  std::vector<std::string> filters = {"All Files", "*"}, bool confirm_overwrite = true)
		: file_dialog(type::save, title, default_path, filters, false, confirm_overwrite)
	{
	}
