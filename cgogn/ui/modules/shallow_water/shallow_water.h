		...
		if (domain_initialized_)
		{
			...
			if (selected_faces_set_)
			{
				...
				for (Face f : *selected_faces_set_)
					ImGui::InputDouble(
						("h"+std::to_string(index_of(*domain_, f))).c_str(),
						&value<Scalar>(*domain_, sw_attributes_.face_h_, f),
						0.01f, 1.0f, "%.3f"
					);
			}
			...
			if (selected_edges_set_)
			{
				...
				for (Edge e : *selected_edges_set_)
				{
					...
					if (ImGui::BeginCombo(("BC_Type_"+std::to_string(index_of(*domain_, e))).c_str(), simulation::shallow_water::bc_name(e_bc_type).c_str()))
					{
					...
					ImGui::InputDouble(
						("BC_Value_"+std::to_string(index_of(*domain_, e))).c_str(),
						&value<Scalar>(*domain_, sw_attributes_.edge_bc_value_, e),
						0.01f, 1.0f, "%.3f"
					);
				}
			}
