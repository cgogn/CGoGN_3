...
class SurfaceDeformation : public ViewModule
{
	...
	struct Parameters
	{
		Parameters() :
			...
			initialized_(false),
			solver_ready_(false),
			vertex_position_init_(nullptr),
			vertex_diff_coord_(nullptr),
			vertex_bi_diff_coord_(nullptr),
			vertex_rotation_matrix_(nullptr),
			vertex_rotated_diff_coord_(nullptr),
			vertex_rotated_bi_diff_coord_(nullptr),
...
private:
	...
	void initialize_mesh_data(MESH* m)
	{
		...
		foreach_cell(*m, [&] (Vertex v) -> bool { value<uint32>(*m, p.vertex_index_, v) = nb_vertices++; return true; });
	...
	void build_solver(MESH* m)
	{
		...
		if (p.initialized_ && !p.solver_ready_ &&
			p.selected_free_vertices_set_ && p.selected_free_vertices_set_->size() > 0 &&
			p.selected_handle_vertices_set_ && p.selected_handle_vertices_set_->size() > 0)
		{
			...
			foreach_cell(*m, [&] (Vertex v) -> bool
			{
				if (p.selected_handle_vertices_set_->contains(v))
				{
					...
					{
						if (!p.selected_handle_vertices_set_->contains(av) && !p.selected_free_vertices_set_->contains(av))
							handle_ok = false;
						return handle_ok;
					});
				}
			...
			p.working_cells_->template build<Edge>([&] (Edge e) -> bool
			{
				auto vertices = incident_vertices(*m, e);
				return (
					working_vertices_marker.is_marked(vertices[0]) &&
