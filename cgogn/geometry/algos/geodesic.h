...
struct Joint
{
	 ...
	 */
	Joint(IntrinsicTriangulation& intr, std::list<Edge>::iterator edge_iterator_a,
 ...
 */
inline bool isInPath(const CMap2& mesh, std::list<Edge>& path, const Dart& d)
{
	for (auto e : path)
	{
 ...
 */
inline bool isFlexible(IntrinsicTriangulation& intr, std::list<Edge>& path, const Joint& joint)
{
 ...
 */
inline JointsPriorityQueue buildJointList(IntrinsicTriangulation& intr, std::list<Edge>& path, bool loop = false)
{
 ...
 */
std::list<Edge> flip_out(IntrinsicTriangulation& intr, const Joint& j)
{
 ...
 */
void geodesic_path(IntrinsicTriangulation& intr, std::list<Edge>& path, int iteration = 100, bool loop = false)
{
	const CMap2& mesh = intr.getMesh();
	...
	while (joints_priority_queue.size() > 0 && iteration > 0)
	{
		...
		// update the priority queue with the new shorter path, in fact rebuild the whole path as every joint
		// flexibility need to be checked
