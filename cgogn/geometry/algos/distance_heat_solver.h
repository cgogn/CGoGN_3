...
DistanceHeatSolver(MESH& mesh, std::shared_ptr<Attribute<Vec3>> position, std::shared_ptr<Attribute<Vec3>> face_normal, double time_multiplier = 1.0){
	this->_mesh = &mesh;
	...
	Eigen::SparseMatrix<Scalar> heatOp = Eigen::SparseMatrix<Scalar, Eigen::ColMajor>(areas.asDiagonal()) - (t*Lc);
...
void solve(Attribute<Scalar>* out_heat_distance, ui::CellsSet<MESH, Vertex>& cell_source){
	...
	cell_source.foreach_cell([&](Vertex vec) {
		source(value<uint32>(*_mesh, vertex_index.get(), vec)) = 1;
	});
	...
	foreach_cell(*_mesh, [&](Face f) -> bool {
		...
		for (int i = 0; i < 3; i++){
			...
			Vec3 ei = value<Vec3>(*_mesh, position, vertices[(i+1)%3])
					- value<Vec3>(*_mesh, position, vertices[i]);
			grad += Ni.cross(ei) * Ui;
		}
	...
	foreach_cell(*_mesh, [&](Vertex v) -> bool {
		b(value<uint32>(*_mesh, vertex_index.get(), v)) = 
			value<Scalar>(*_mesh, integrated_divergence, v);
		return true;
	});
	...
	parallel_foreach_cell(*_mesh, [&](Vertex v) -> bool {
		value<Scalar>(*_mesh, out_heat_distance, v) =
			phi(value<uint32>(*_mesh, vertex_index.get(), v));
		return true;
	});
