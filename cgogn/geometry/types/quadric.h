...
public:
	...
	inline Slab_Quadric(const Vec4& p1, const Vec4& p2, const Vec4& p3)
	{
		...
		_A = (n1.transpose())*n1 + (n2.transpose())* n2;
		_b = -2 * _A * (p1);
	...
	inline Slab_Quadric(const Vec4& p1, const Vec4& p2)
	{
		...
		auto [n1,n2] = slab_normal(p1, p2, p3);
		Eigen::AngleAxisd aa(0.5 * M_PI, (p2 - p1).normalized());
		Eigen::Quaterniond q(aa);
		Eigen::Quaterniond q_n1, q_n2;
		q_n1.w() = 0;
		q_n2.w() = 0;
		q_n1.vec() = n1;
		q_n2.vec() = n2;
		...
		Vec3 r_n1 = rotated_n1.vec();
		Vec3 r_n2 = rotated_n2.vec();
		_A = (n1.transpose()) * n1 + (n2.transpose()) * n2;
		_b = -2 * _A * (p1);
		_c = p1.transpose() * _A * p1;
		_A += (r_n1.transpose()) * r_n1 + (r_n2.transpose()) * r_n2;
		_b += -2 * _A * (p1);
		_c += p1.transpose() * _A * p1;
	}
	...
	inline std::pair<Vec4, Vec4> slab_normal(const Vec4& p1, const Vec4& p2, const Vec4& p3)
	{
		...
		double t = ((inter2.x() - inter1.x()) * q2.y() -
					(inter2.y() - inter1.y()) * q2.x()) / (q1.x() * q2.y() - q1.y() * q2.x());
		Vec3 p = inter1 + t * q1;
