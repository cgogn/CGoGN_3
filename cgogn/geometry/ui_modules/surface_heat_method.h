...
public:
	SurfaceHeatMethod(const App& app)
		: Module(app, "SurfaceHeatMethod (" + std::string{mesh_traits<MESH>::name} + ")"), selected_mesh_(nullptr),
		  selected_vertex_position_(nullptr), selected_vertices_set_(nullptr), euclidean_dist_vert(nullptr),
		  euclidean_dist_face(nullptr), geo_dist_vert(nullptr), geo_dist_face(nullptr), heat_distance(nullptr),
		  selected_face_normal_(nullptr)
	{
	}
	~SurfaceHeatMethod()
	{
...
protected:
	...
	*/
	...
	Eigen::SimplicialLDLT<Eigen::SparseMatrix<Scalar>>* compute_heat_solver(
		MESH& mesh, std::shared_ptr<Attribute<Vec3>> position, std::shared_ptr<Attribute<uint32>> vertex_index,
		uint32 nb_vertices, Eigen::SparseMatrix<Scalar, Eigen::ColMajor>* Lc,
		...
		bool keep_local_attribute = false)
	{
		...
		if (time_logger != nullptr)
			time_logger->push_back(std::pair<std::string, std::chrono::steady_clock::time_point>(
		...
		*Lc = geometry::cotan_operator_matrix(mesh, vertex_index.get(), position.get());
		...
		Eigen::SimplicialLDLT<Eigen::SparseMatrix<Scalar>>* solver =
			new Eigen::SimplicialLDLT<Eigen::SparseMatrix<Scalar>>(
	...
	void heat_compute(MESH& mesh, std::shared_ptr<Attribute<Scalar>> out_heat_distance,
					  std::shared_ptr<Attribute<Vec3>> position, std::shared_ptr<Attribute<Vec3>> face_normal,
					  std::shared_ptr<Attribute<uint32>> vertex_index, uint32 nb_vertices,
					  Eigen::SimplicialLDLT<Eigen::SparseMatrix<Scalar>>& solver, CellsSet<MESH, Vertex>& cell_source,
					  ...
					  bool keep_local_attribute = false)
	{
		...
		cell_source.foreach_cell([&](Vertex vec) { source(value<uint32>(mesh, vertex_index.get(), vec)) = 1; });
		...
		foreach_cell(mesh, [&](Face f) -> bool {
			...
			for (int i = 0; i < 3; i++)
			{
				...
				Vec3 ei = value<Vec3>(mesh, position, vertices[(i + 1) % 3]) - value<Vec3>(mesh, position, vertices[i]);
				grad += Ni.cross(ei) * Ui;
			}
		...
		foreach_cell(mesh, [&](Vertex v) -> bool {
			b(value<uint32>(mesh, vertex_index.get(), v)) = value<Scalar>(mesh, integrated_divergence, v);
			return true;
		});

		auto geo_solver =
			Eigen::SimplicialLDLT<Eigen::SparseMatrix<Scalar>>(Eigen::SparseMatrix<Scalar, Eigen::ColMajor>(Lc));
		...
		parallel_foreach_cell(mesh, [&](Vertex v) -> bool {
			value<Scalar>(mesh, out_heat_distance, v) = phi(value<uint32>(mesh, vertex_index.get(), v));
			return true;
		});
	...
	void heat_compute_one_step(
		MESH& mesh, std::shared_ptr<Attribute<Scalar>> out_heat_distance, std::shared_ptr<Attribute<Vec3>> position,
		std::shared_ptr<Attribute<Vec3>> face_normal, CellsSet<MESH, Vertex>& cell_source, double time_multiplier = 1.0,
		std::vector<std::pair<std::string, std::chrono::steady_clock::time_point>>* time_logger = nullptr,
		bool keep_local_attribute = false)
	{
		...
		Eigen::SparseMatrix<Scalar, Eigen::ColMajor> Lc;
		auto solver = compute_heat_solver(mesh, position, vertex_index, nb_vertices, &Lc, time_multiplier, time_logger,
										  keep_local_attribute);

		heat_compute(mesh, out_heat_distance, position, face_normal, vertex_index, nb_vertices, *solver, cell_source,
					 Lc, time_logger, keep_local_attribute);
		// heat_compute(mesh, out_heat_distance, position, face_normal, vertex_index, nb_vertices, *solver, cell_source,
		// Lc, time_multiplier, time_logger, keep_local_attribute);
		if (!keep_local_attribute)
		{
			...
			/*
						imgui_combo_attribute<Vertex, uint32>(
							*selected_mesh_, selected_vertex_index_, "Vertex Index",
							[&](const std::shared_ptr<Attribute<uint32>>& attribute) { selected_vertex_index_ =
			   attribute; });
							*/
				...
				{
					{
						...
						{
							time_log.clear();
							heat_compute_one_step(*selected_mesh_, heat_distance, selected_vertex_position_,
												  selected_face_normal_, *selected_vertices_set_, t_multiplier,
												  &time_log, true);
						...
						if (ImGui::Button("(Re)Generate DistanceHeatSolver"))
						{
							...
							time_log.clear();
							time_log.push_back(std::pair<std::string, std::chrono::steady_clock::time_point>(
								"start", std::chrono::steady_clock::now()));
							distanceHeatSolver = new geometry::DistanceHeatSolver<MESH>(
								*selected_mesh_, selected_vertex_position_, selected_face_normal_, t_multiplier);
							time_log.push_back(std::pair<std::string, std::chrono::steady_clock::time_point>(
								"(Re)Generate DistanceHeatSolver", std::chrono::steady_clock::now()));
						}
						if (distanceHeatSolver != nullptr)
						{
							if (ImGui::Button("Compute From Source"))
							{
								...
								{
									heat_distance =
										get_or_add_attribute<Scalar, Vertex>(*selected_mesh_, "heat_distance");
								}
								time_log.clear();
								time_log.push_back(std::pair<std::string, std::chrono::steady_clock::time_point>(
									"start", std::chrono::steady_clock::now()));
								...
								time_log.push_back(std::pair<std::string, std::chrono::steady_clock::time_point>(
									"Compute From Source", std::chrono::steady_clock::now()));
								mesh_provider_->emit_attribute_changed(*selected_mesh_, heat_distance.get());
							}
						...
						if (ImGui::Button("Compute heat one step")){
							time_log.clear();
							heat_compute_one_step(*selected_mesh_, heat_distance, selected_vertex_position_,
						selected_face_normal_, *selected_vertices_set_, t_multiplier, &time_log, true);
							mesh_provider_->emit_attribute_changed(*selected_mesh_, heat_distance.get());
						}
						if (selected_vertex_index_ != nullptr && nb_vertices != 0){
							if (ImGui::Button("Compute Heat")){
								time_log.clear();
								Eigen::SparseMatrix<Scalar, Eigen::ColMajor> Lc;
								auto solver = compute_heat_solver(*selected_mesh_, selected_vertex_position_,
						selected_vertex_index_, nb_vertices, &Lc, t_multiplier, &time_log, false);

								heat_compute(*selected_mesh_, heat_distance, selected_vertex_position_,
						selected_face_normal_,selected_vertex_index_, nb_vertices, *solver, *selected_vertices_set_, Lc,
						&time_log, false); mesh_provider_->emit_attribute_changed(*selected_mesh_, heat_distance.get());
								delete solver;
				...
				if (time_log.size() > 0)
				{
					if (ImGui::BeginTable("heat_time_log", 2, ImGuiTableFlags_Resizable))
					{
						for (int i = 1; i < time_log.size(); i++)
						{
							...
							ImGui::TextUnformatted(time_log[i].first.c_str());
							ImGui::TableNextColumn();
							std::string dtext = std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(
																   time_log[i].second - time_log[i - 1].second)
																   .count()) +
												std::string(" ms");
							ImGui::TextUnformatted(dtext.c_str());
						}
						ImGui::EndTable();
					}
				}
