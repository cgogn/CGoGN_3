...
bool import_CGR(MESH& m, const std::string& filename)
{
...
template <typename MESH>
void export_CGR(MESH& m, const typename mesh_traits<MESH>::template Attribute<geometry::Vec3>* vertex_position,
				const typename mesh_traits<MESH>::template Attribute<geometry::Scalar>* vertex_radius,
				const std::string& filename)
{
	...
	foreach_cell(m, [&](Vertex v) -> bool {
		...
		geometry::Scalar radius = value<geometry::Scalar>(m, vertex_radius, v);
