...
Dart add_dart(MRCmap3& m)
{
	...
	if(m.current_level() > m.cph().maximum_level())
		m.cph().maximum_level(m.current_level());
	return d;
}
...
uint32 edge_level(const MRCmap3& m, Dart d){
	...
	return std::max(m.cph().dart_level(d),m.cph().dart_level(phi1(m,d)));
}

Dart edge_youngest_dart(const MRCmap3& m, Dart d){
	cgogn_message_assert(m.cph().dart_level(d) <= m.current_level(),
				 "Access to a dart introduced after current level");
	if(m.cph().dart_level(d) > m.cph().dart_level(phi2(m,d)))
		return d;
	return phi2(m,d);
}
...
uint32 face_level(const MRCmap3& m, Dart d){
	...
	Dart it = d;
	Dart old = it;
	uint32 l_old = m.cph().dart_level(old);
	uint32 fLevel = edge_level(m,it);
	do
	{
		it = phi1(m,it);
		uint32 dl = m.cph().dart_level(it);
	...
	uint32 nbSubd = 0;
	it = old;
	uint32 eId = m2.cph().edge_id(old);
	unsigned int init_dart_level = m2.cph().dart_level(it);
	do
	{
		++nbSubd;
		it = phi1(m2,it);
	} while(m2.cph().edge_id(it) == eId && m2.cph().dart_level(it) != init_dart_level);
...
Dart face_origin(const MRCmap3& m, Dart d){
	cgogn_message_assert(m.cph().dart_level(d) <= m.current_level(),
				 "Access to a dart introduced after current level");
	Dart p = d;
	uint32 pLevel = m.cph().dart_level(p);
	MRCmap3 m2(m,pLevel);
	while(pLevel > 0){
		p = face_oldest_dart(m2,p);
		pLevel = m2.cph().dart_level(p);
...
uint32 volume_level(const MRCmap3& m, Dart d){
	...
	Dart oldest = d ;
	uint32 lold = m.cph().dart_level(oldest);
	...
	foreach_incident_face(m,MRCmap3::Volume(d),[&](MRCmap3::Face d)->bool{
		...
		if(m.cph().dart_level(old) < lold){
			oldest = old;
			lold = m.cph().dart_level(old);
		}
		return true;
	});
...
bool face_is_subdivided(const MRCmap3& m, Dart d){
	...
	MRCmap3 m2(m,m.current_level()+1);
	if(m2.cph().dart_level(phi1(m2,d)) == m2.current_level()
		&& m2.cph().edge_id(phi1(m2,d)) != m2.cph().edge_id(d))
		subd = true;
...
bool face_is_subdivided_once(const MRCmap3& m, Dart d){
	...
	do
	{
		if(m2.cph().dart_level(phi1(m2,fit)) == m2.current_level()
			&& m2.cph().edge_id(phi1(m2,fit)) != m2.cph().edge_id(fit))
		{
			subd = true ;
			if(m3.cph().dart_level(phi1(m3,fit)) == m3.current_level()
				&& m3.cph().edge_id(phi1(m3,fit)) != m3.cph().edge_id(fit))
				subdOnce = false ;
		}
		++degree ;
		fit = phi1(m,fit) ;
	...
	if(degree == 3 && subd)
	{
		...
		if(m3.cph().dart_level(phi1(m3,cf)) == m3.current_level()
			&& m3.cph().edge_id(phi1(m3,cf)) != m3.cph().edge_id(cf))
		subdOnce = false ;
	}
...
bool volume_is_subdivided(const MRCmap3& m, Dart d){
	...
	bool subd = false;
	MRCmap3 m2(m,m.current_level()+1);
	if(faceAreSubdivided && m2.cph().dart_level(phi<112>(m2,d)) == m2.current_level() && m2.cph().face_id(phi<112>(m2,d)) != m2.cph().face_id(d))
		subd = true;
