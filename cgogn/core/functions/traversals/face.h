...
auto foreach_incident_face(const MESH& m, typename mesh_traits<MESH>::Vertex v, const FUNC& func)
...
template <typename MESH, typename FUNC>
auto foreach_incident_face(const MESH& m, typename mesh_traits<MESH>::Edge e, const FUNC& func)
	-> std::enable_if_t<std::is_convertible_v<MESH, CMap2>>
{
	using Face = typename mesh_traits<MESH>::Face;
	static_assert(is_func_parameter_same<FUNC, Face>::value, "Wrong function cell parameter type");
