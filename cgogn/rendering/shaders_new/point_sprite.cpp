...
#include <cgogn/rendering/shaders_new/point_sprite.h>
...
PointSprite* PointSprite::instance_ = nullptr;
PointSpriteColor* PointSpriteColor::instance_ = nullptr;
PointSpriteSize* PointSpriteSize::instance_ = nullptr;
PointSpriteColorSize* PointSpriteColorSize::instance_ = nullptr;
...
static const char* geometry_shader_source = "layout (points) in;\n"
...
PointSprite::PointSprite()
{
	...
	const char* geometry_shader_source = R"(
		...
		void corner(vec4 center, float x, float y)
		{
			...
			vec4 position = center + vec4(point_size * x, point_size * y, 0.0, 0.0);
			gl_Position = projection_matrix * pos;
			EmitVertex();
		}
		...
		void main()
		{
			vec4 center = model_view_matrix * gl_in[0].gl_Position;
			sphere_center = center.xyz;
			corner(center, -1.4,  1.4);
			corner(center, -1.4, -1.4);
			corner(center,  1.4,  1.4);
			corner(center,  1.4, -1.4);
			EndPrimitive();
...
void PointSpriteParam::set_uniforms() override
{
	shader_->set_uniforms_values(color_, ambiant_color_, light_position_, size_, clipping_plane_, clipping_plane_2_);
}

PointSpriteColor::PointSpriteColor()
{
...
PointSpriteSize::PointSpriteSize()
{
	std::string bs("#version 150\n#define WITH_COLOR 0\n#define WITH_SIZE 1\n");
...
PointSpriteColorSize::PointSpriteColorSize()
{
	std::string bs("#version 150\n#define WITH_COLOR 1\n#define WITH_SIZE 1\n");
