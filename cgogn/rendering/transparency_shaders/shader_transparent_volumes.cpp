...
const char* ShaderTransparentVolumes::vertex_shader_source_ = "#version 330\n"
															  "in vec3 vertex_pos;\n"
															  "void main()\n"
															  "{\n"
...
ShaderParamTransparentVolumes::ShaderParamTransparentVolumes(ShaderTransparentVolumes* sh)
	: ShaderParam(sh), color_(255, 0, 0), plane_clip_(0, 0, 0, 0), plane_clip2_(0, 0, 0, 0),
	  light_position_(10.0f, 100.0f, 1000.0f), explode_factor_(0.8f), bf_culling_(false), lighted_(true)
{
}
...
void ShaderParamTransparentVolumes::set_position_vbo(VBO* vbo_pos)
{
	...
	ogl->glVertexAttribPointer(ShaderTransparentVolumes::ATTRIB_POS, vbo_pos->vector_dimension(), GL_FLOAT, GL_FALSE, 0,
							   0);
	vbo_pos->release();
	vao_->release();
	shader_->release();
