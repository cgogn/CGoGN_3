...
const char* ShaderPhongTransp::vertex_shader_source_ = "#version 330\n"
													   "in vec3 vertex_pos;\n"
...
ShaderParamPhongTransp::ShaderParamPhongTransp(ShaderPhongTransp* sh)
	: ShaderParam(sh), front_color_(250, 0, 0), back_color_(0, 250, 0), ambiant_color_(5, 5, 5),
	  specular_color_(255, 255, 255), specular_coef_(100), light_pos_(10, 100, 1000), bf_culling_(false)
{
}
...
void ShaderParamPhongTransp::set_normal_vbo(VBO* vbo_normal)
{
	...
	ogl->glVertexAttribPointer(ShaderPhongTransp::ATTRIB_NORM, vbo_normal->vector_dimension(), GL_FLOAT, GL_FALSE, 0,
							   0);
	vbo_normal->release();
	vao_->release();
	shader_->release();
