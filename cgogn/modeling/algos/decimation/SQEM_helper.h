...
struct ClusteringSQEM_Helper
{
	...
	Scalar cluster_cost(const std::vector<Vertex>& cluster_vertices, const Vec4& p)
	{
	...
	Scalar cluster_cost(const std::vector<Vertex>& cluster_vertices, const Vec4& p, const Attribute<Scalar>* weights)
	{
		if (cluster_vertices.size() == 0)
			return Scalar(0.0);
		Spherical_Quadric q;
	...
	Scalar vertex_cost(Vertex v, const Vec4& p, const Attribute<Scalar>* weights)
	{
		Spherical_Quadric q = value<Spherical_Quadric>(m_, vertex_quadric_, v) * value<Scalar>(m_, weights, v);
		return q.eval(p);
	}

	Scalar vertex_cost(Vertex v, const Vec4& s)
	{
		return value<Spherical_Quadric>(m_, vertex_quadric_, v).eval(s);
	}

	bool optimal_sphere(const std::vector<Vertex>& cluster_vertices, Vec4& sphere, const Attribute<Scalar>* weights)
	{
		Spherical_Quadric q;
		for (Vertex v : cluster_vertices)
			q += value<Spherical_Quadric>(m_, vertex_quadric_, v) * value<Scalar>(m_, weights, v);
	...
	bool optimal_sphere(const std::vector<Vertex>& cluster_vertices, Vec4& sphere)
	{
		Spherical_Quadric q;
		for (Vertex v : cluster_vertices)
			q += value<Spherical_Quadric>(m_, vertex_quadric_, v);
	...
	Mat4 hessian(Vertex v)
	{
		const Spherical_Quadric& q = value<Spherical_Quadric>(m_, vertex_quadric_, v);
		return q._A.transpose();
	}

	Vec4 jacobian(Vertex v, const Vec4& s)
	{
		const Spherical_Quadric& q = value<Spherical_Quadric>(m_, vertex_quadric_, v);
		return q._A * s - q._b;
	}
