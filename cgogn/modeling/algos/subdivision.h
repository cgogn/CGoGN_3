...
template<typename MESH>
auto surfaceAdjacentFace(const MESH& m,Dart d)
...
template<typename MESH>
auto surfaceEdgePointMask(const MESH& m,Dart d,
...
auto volumePointRule(const MESH& m,
...
auto facePointRule(const MESH& m,const std::vector<Dart> &p_point, const std::vector<Dart> &q_point,
				...
				const std::vector<Dart> &t_point,typename mesh_traits<MESH>::template Attribute<T>* attribute)
-> std::enable_if_t<std::is_convertible_v<MESH&, CMapBase&>,Vec3>
{
	static const float _W_ = 1.f/16;
	if (q_point.size() == 0) {
...
template<typename T,typename MESH>
auto surfaceFacePointRule(const MESH& m,const std::vector<Dart> &p_point,
					   const std::vector<Dart> &q_point,
					   typename mesh_traits<MESH>::template Attribute<T>* attribute)
-> std::enable_if_t<std::is_convertible_v<MESH&, CMapBase&>,Vec3>
{
...
auto surfaceEdgePointRule(const MESH& m,const std::vector<Dart> &p_point, const std::vector<Dart> &q_point,
					   const std::vector<Dart> &r_point,typename mesh_traits<MESH>::template Attribute<T>* attribute)
-> std::enable_if_t<std::is_convertible_v<MESH&, CMapBase&>,Vec3>
{
	static const float _W_ = 1.f/16;
